...
def train_model(model, train_ys, train_xs, dev_ys, dev_xs, args):
    ...
    for i in range(args.iterations):
        for n in range(N):
            ...
            reduced_w2 = w2[:, 0:args.hidden_dim]
            del1 = del2 * (reduced_w2.T * sigmoid_d(cache["z1"]))
            dw1 = np.dot(del1, x_vector.T)
            #print(w2, reduced_w2, reduced_w2.shape)
            ...
            #del1 = np.dot(del2, reduced_w2).T *  sigmoid_d(cache["z1"])
        ...
        print(calculate_error(z2, train_ys.T)/N)

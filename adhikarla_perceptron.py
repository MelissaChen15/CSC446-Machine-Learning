...
#DATA_PATH = "/home/hoover/u8/sadhika4/ml_hw2/inputs" 
...
def perceptron(train_ys, train_xs, dev_ys, dev_xs, args):
    ...
    train_acc_history = list() # List to keep track of the training accuracy achieved after each iteration
    dev_acc_history = list()   # List to keep track of the development accuracy achieved after each iteration (if development set available)

    # Perceptron algorithm Loop begins 
    for i in range(args.iterations):
        for n in range(N):
            if train_ys[n] * predict(train_xs[n], weights) <= 0:
                weights = weights + args.lr * train_ys[n] * train_xs[n]

        # Below code is just to keep track of the training and development accuracy and plotting the results
    # CAN BE COMMENTED SAFELY (ALGORITHM WILL STILL WORK)
        tr_accuracy = test_accuracy(weights, train_ys, train_xs)
        train_acc_history.append(tr_accuracy)
        if not args.nodev:
            dev_accuracy = test_accuracy(weights, dev_ys, dev_xs)
            dev_acc_history.append(dev_accuracy)
            #if i%10 == 0:
                #print("Iteration-{0}; Train Accuracy:{1}; Dev Accuracy:{2}".format(i+1, tr_accuracy, dev_accuracy))   
        #elif i%10 == 0:
            #print("Iteration-{0}; Train Accuracy:{1}".format(i+1, test_accuracy(weights, train_ys, train_xs)))
    ...
    # plt.legend()
    # plt.savefig("adhikarla_perceptron.png")
    # plt.show()
    # CAN BE COMMENTED SAFELY (ALGORITHM WILL STILL WORK)
    return weights

def predict(data, weights):
    y_pred = np.dot(weights.T, data)
    y_hat = 0
    if y_pred < 0 :
        y_hat = -1
    elif y_pred > 0:
...
def test_accuracy(weights, test_ys, test_xs):
    ...
    for n in range(total):
        if test_ys[n] == predict(test_xs[n], weights):
            s+=1
    accuracy = s/total
    return accuracy
